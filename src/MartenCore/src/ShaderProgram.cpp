...
namespace MartenEngine {
    ...
    bool create_shader(const char* source, const GLenum shader_type, GLuint& shader_id) {
        ...
        if (success == GL_FALSE) {
            ...
            MartenLogger::CRIT("[create_shader] Compilation failed: \n" + std::string(info_log));
            return false;
        }
        return true;
    }
    ...
    ShaderProgram::ShaderProgram(const char* vertex_shader_src, const char* fragment_shader_src) {
        ...
        if (!create_shader(vertex_shader_src, GL_VERTEX_SHADER, vertex_shader_id)) {
            MartenLogger::CRIT("[Vertex Shader] Compilatoin time failed");
        ...
        if (!create_shader(fragment_shader_src, GL_FRAGMENT_SHADER, fragment_shader_id)) {
            MartenLogger::CRIT("[Fragment Shader] Compilation time failed");
            glDeleteShader(vertex_shader_id);
        ...
        if (success == GL_FALSE) {
            ...
            MartenLogger::CRIT("[SHADER PROGRAM] Link-time failed: \n" + std::string(info_log));
